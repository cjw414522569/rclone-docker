...
# 检查必要的目录和权限
echo "检查目录和权限..."
for dir in "/media" "/cache" "/config"; do
    if [ ! -d "$dir" ]; then
        echo "创建目录: $dir"
        mkdir -p "$dir"
    fi
...
if [ ! -f "$RCLONE_CONF" ]; then
    ...
    echo "请运行docker exec -it rclone-mount rclone --config="/config/.rclone.conf" config"
		tail -f /dev/null
    exit 1
fi
...
# 启动 rclone 挂载
echo "启动 rclone 挂载..."
exec s6-setuidgid abc mount "$MOUNT_PATH":/"$REMOTE_NAME" /mnt/rclone \
    --daemon \
    --allow-other \
